...
const TeacherListPage = async ({
  ...
  const renderRow = (item: TeacherList) => (
    ...
    >
      ...
      <td className="hidden md:table-cell">
        {item.classes.map((class_) => class_.name).join(", ")}
      </td>
      ...
      <td>
        <div className="flex items-center gap-2">
          <Link href={`/list/teachers/${item.id}`}>
            <button className="w-7 h-7 flex items-center justify-center rounded-full bg-lamaSky hover:bg-lamaSky/80 transition-colors" aria-label="View teacher details">
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold text-white">All Teachers</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch />
          <div className="flex items-center gap-4 self-end">
            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-700 hover:bg-gray-600 transition-colors" aria-label="Filter teachers">
              <Image src="/filter.png" alt="" width={14} height={14} className="invert" />
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-700 hover:bg-gray-600 transition-colors" aria-label="Sort teachers">
